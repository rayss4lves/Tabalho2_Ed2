...
Inglesbin* createNode(const char* palavraIngles, int unidade) {
    ...
    if (newNode != NULL) {
        strcpy(newNode->palavraIngles, palavraIngles);
...
Info criaInfo(char *palavra, char *palavraIngles, int unidade) {
    ...
    if (!info.palavraPortugues) {
        perror("Erro ao alocar memória para palavra em português");
        exit(EXIT_FAILURE);
    }
...
void exibir_traducoes_unidade(const Portugues23 *raiz, int unidade){
    if(raiz != NULL){
        ...
        exibir_traducoes_unidade(raiz->cent, unidade); // Subárvore central para árvore 2-3
...
//     Portugues23 **nodo = arvore;  // Inicializa a árvore de busca

//     // Verifica se a árvore e o primeiro nó não são NULL
//     while (*nodo) {
//         // Verifica se o valor está no primeiro ou segundo campo do nó
//         if (strcmp(valor, (*nodo)->info1.palavraPortugues) == 0 || 
//             ((*nodo)->nInfos == 2 && strcmp(valor, (*nodo)->info2.palavraPortugues) == 0)) {
//             return nodo;  // Se encontrou o valor, retorna o ponteiro para o nó
//         }

//         // Atualiza `irmao` para o filho do meio (cent) se o valor não foi encontrado
//         *irmao = (*nodo)->cent;

//         // Se o valor for menor que o primeiro campo, vai para o nó esquerdo
//         if (strcmp(valor, (*nodo)->info1.palavraPortugues) < 0) {
//             nodo = &((*nodo)->esq);
//         } 
//         // Se o valor for maior que o segundo campo, vai para o nó direito
//         else if ((*nodo)->nInfos == 2 && strcmp(valor, (*nodo)->info2.palavraPortugues) > 0) {
//             nodo = &((*nodo)->dir);
//         } 
//         // Caso contrário, vai para o nó do meio
//         else {
//             nodo = &((*nodo)->cent);
